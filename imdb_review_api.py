...
def get_movie_details(movie_list: list, cmd=None):
    ...
    movie_details = list(
        zip(movie_names, movie_years, image_links, movie_id, movie_links, overviews, genres, directors))
    ...
    if request.method == "POST":
        ...
        if username in users:
            ...
            if len(users):
                ...
                if mongodb:
                    insert_id = collection.insert_one(
...
def movie_search_result():
    if request.method == "POST":
        ...
        return render_template("search_result.html", main_movie=movie, movies=movie_details,
                               movieListForInputField=movies_data["title"].values)
    else:
...
def recommendation():
    ...
    return render_template("recommendation.html", movieListForInputField=movies_data["title"].values,
                           movies=movie_details[1:], main_movie=movie_details[0])
